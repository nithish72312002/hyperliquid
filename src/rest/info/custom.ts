...
export class CustomInfoAPI {
  ...
  private async getSpotMeta(): Promise<SpotMeta> {
    const response = (await this.httpApi.makeRequest({ type: InfoType.SPOT_META })) as SpotMeta;
    return response;
  }

  // Helper method to get spot clearinghouse state
  private async getSpotClearinghouseState(user: string): Promise<SpotClearinghouseState> {
   ...
   */
  async getTransferrableAssets(user: string): Promise<
    Array<{
      coin: string;
      token: number;
      total: string;
  ...
  > {
    ...
    const [clearinghouseState, meta] = await Promise.all([
      this.getSpotClearinghouseState(user),
      this.getSpotMeta(),
    ]);
